...
function releaseGateSub(d: DashboardData): string {
  const r = d.eval_report as EvalReport | null
  if (r?.deterministic != null && r?.judge != null) {
    return `det ${r.deterministic} · judge ${r.judge}`
  }
  return 'run make gate'
}

/** Byte-faithful port of waku `archSVG` — node/edge ids frozen; no onclick (React handles clicks). */
export function archSvg(d: DashboardData): string {
  const s = d.stats
  const box = (x: number, y: number, w: number, h: number, title: string, sub: string, cls = '', nid = '') =>
    `<g class="node ${cls}" ${nid ? `data-node="${nid}"` : ''}>
  ...
  const lbl = (x: number, y: number, t: string) => `<text class="grp" x="${x}" y="${y}">${t}</text>`
  const flow = (d2: string, cls = '', eid = '') =>
    `<path class="flow ${cls}" ${eid ? `data-edge="${eid}"` : ''} d="${d2}"/>`
  const flowLbl = (x: number, y: number, t: string, anchor = 'start') =>
    `<text class="fl" x="${x}" y="${y}" text-anchor="${anchor}">${t}</text>`
  ...
  return `<div style="overflow-x:auto"><svg viewBox="0 -10 1044 674" class="arch" role="img">
    ...
    ${box(32, 72, 128, 56, 'Gateway', 'cli · dashboard', '', 'gateway')}
    ${flow('M160 100 L192 100', '', 'e-gw-wm')}
    ${box(192, 72, 144, 56, 'Working memory', 'assembled per turn', '', 'wm')}

    <rect class="loopbox" x="370" y="56" width="168" height="166" rx="12"/>
    ${lbl(384, 48, 'LOOP')}
    ${box(384, 72, 140, 50, 'LLM agent', 'reason', '', 'llm')}
    ${box(384, 152, 140, 52, 'Tools', 'create_event…', '', 'tools')}
    ${flow('M448 122 L448 152')}${flow('M470 152 L470 122')}
    ${flowLbl(456, 141, 'act')}
    ${flow('M336 100 L370 100', '', 'e-wm-loop')}
    ${flow('M538 100 L558 106')}${flowLbl(542, 93, 'reply')}
    ${box(558, 84, 104, 52, 'Reply', '→ back to you', '', 'reply')}
    ...
    <path class="gate node" data-node="gate" d="M264 250 L340 296 L264 342 L188 296 Z"/>
    ...
    ${box(44, 452, 208, 72, 'Procedural', 'how to act · SKILL.md · ' + d.skills.length + ' skill(s)', '', 'procedural')}
    ${box(264, 452, 204, 72, 'Semantic · FTS5', 'durable facts · ' + d.facts.length + ' facts', '', 'semantic')}
    ${box(480, 452, 132, 72, 'Episodic', d.episodes.length + ' episodes', '', 'episodic')}

    <!-- consolidation writes back into memory -->
    ${box(44, 576, 384, 52, 'Consolidation · every ' + d.consolidate_every + ' exchanges', d.chat_pending + '/' + d.consolidate_every * 2 + ' queued → distilled into facts', '', 'consolidation')}
    ...
    ${box(752, 92, 250, 50, 'Trace', s.trace_files + ' file(s) · always on', '', 'trace')}
    ${flow('M878 142 L878 156')}
    ${box(752, 156, 250, 50, 'Eval', 'deterministic + judge', '')}
    ${flow('M878 206 L878 220')}
    ${box(752, 220, 250, 50, 'Release gate', releaseGateSub(d), '')}
    ${flow('M878 270 L878 284')}
    ${box(752, 284, 250, 50, 'Release', 'new prompt · model · config', '')}
...
/** Live animation map — node/edge ids MUST match archSvg `data-node` / `data-edge` above. */
...
export const STAGE: Record<string, StageSpec> = {
  ...
  turn_end: {
    nodes: ['reply', 'trace'],
    edges: ['e-reply-trace', 'e-reply-save'],
    label: 'reply',
  },
  consolidation: {
    nodes: ['consolidation', 'semantic'],
    edges: ['e-consol-sem'],
