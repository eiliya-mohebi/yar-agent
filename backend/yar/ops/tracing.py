...
1. JSONL, always on: every turn appends readable lines to
   .yar/traces/<date>.jsonl. A trace is just "what happened, in order" —
...
2. OpenTelemetry spans, when OTEL_EXPORTER_OTLP_ENDPOINT is set: the same
   events as a span tree any OTel backend can render. For a local dashboard:
       ...
       OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4317 uv run yar
...
class TraceEncodingError(UnicodeError):
    ...
    def __init__(self, path: Path):
        ...
        super().__init__(
            ...
            "Yar version using a non-UTF-8 encoding. Move it out of the traces directory "
            "and keep it as a backup, then restart Yar if it is today's trace. The file "
            "was not modified."
        )
...
class Tracer:
    ...
    def _init_otel(self, settings: Settings):
        ...
        try:
            ...
            provider = TracerProvider(resource=Resource.create({"service.name": "yar-agent"}))
            ...
            return trace.get_tracer("yar")
        except ImportError:
            print(
                "(tracing) OTEL endpoint set but opentelemetry not installed — "
                "uv sync --extra tracing. JSONL tracing still on."
            )
            return None
    ...
    def _record_usage(self, event: dict) -> None:
        ...
        record of what you've actually spent — never wiped by default. Tokens are
        the ground truth; dollar cost is derived later from the price map."""
    ...
    def event(self, kind: str, event: dict) -> None:
        ...
        if kind == "llm":
            ...
            event = {"provider": "openai", "model": self.settings.model or "", **event}
        ...
        if self._otel_tracer and self._span_ctx is not None:
            with self._otel_tracer.start_as_current_span(
                ...
                attributes={
                    "openinference.span.kind": {"llm": "LLM", "tool": "TOOL"}.get(
                        kind, "CHAIN"
                    ),
                    **{f"yar.{k}": json.dumps(v, default=str) for k, v in event.items()},
                },
            ):
                pass
