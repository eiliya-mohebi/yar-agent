...
single-model trace, so it must not land in chat_log / facts / calendar.

File: ``<home>/compare/history.jsonl`` — newest last, capped to the most recent
``MAX_RUNS`` races. This module is the single owner of that file.
"""
...
def save_runs(home: Path, runs: list[dict]) -> None:
    """Rewrite the (capped) history file."""
    path = _path(home)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        "\n".join(json.dumps(r, ensure_ascii=False) for r in runs[-MAX_RUNS:]) + "\n",
        encoding="utf-8",
    )


def clear(home: Path) -> None:
    """Wipe the compare history — only the arena's own log."""
...
def aggregate(runs: list[dict]) -> list[dict]:
    """Per-model scoreboard across the given races."""
    acc: dict[str, dict] = {}
    for run in runs:
    ...
    out = [
        {
            ...
            "runs": a["runs"],
            "ok": a["ok"],
            "total_latency_ms": a["lat"],
            "total_tokens_in": a["tin"],
            "total_tokens_out": a["tout"],
            "total_tokens": a["tin"] + a["tout"],
            "cases_passed": a["passed"],
            "cases_scored": a["scored"],
            "quality_n": a["qn"],
