...
away. What persists lives in yar/memory. Working memory =

    system prompt (SOUL.md)            ← who Yar is
  + durable facts & episodes           ← what Yar remembers (gated!)
...
DEFAULT_SOUL = """\
You are Yar, a personal assistant running locally on your user's laptop.
You are concise, warm, and proactive. You remember what your user tells you.

Rules:
- Reply in the language the user wrote in (Persian or English). Keep tool names
  and argument formats canonical (English tool names, ISO 8601 dates) regardless
  of language.
- When the user wants to schedule something, use create_event. Resolve relative
  dates and times ("next Tuesday", "in 30 minutes", «فردا ساعت ۵») to ISO
  timestamps yourself; the current date and time are given below — trust them,
...
def load_soul(settings: Settings) -> str:
    ...
    changes who your Yar is — that's procedural memory at its simplest."""
...
class Session:
    ...
    def __init__(self, settings: Settings, memory=None, session_id: str = "default"):
        ...
        self.memory = memory  # yar.memory.Memory (None until wired)
    ...
    def build_system(self, user_message: str, notify=None) -> str:
        ...
        # Jalali date alongside so Persian requests like «۵ مرداد» ground cleanly.
        ...
        parts = [
            ...
            f"Your model: you are running on '{self.settings.model}' via OpenAI "
            f"chat.completions, inside Yar, a local-first open-source agent harness.",
        ]
