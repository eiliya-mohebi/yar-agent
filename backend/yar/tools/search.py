"""search_web — read the public web so the loop can act on current facts.

Zero new dependencies — just stdlib urllib. Two backends:
  default  DuckDuckGo HTML (no key, no setup — good enough to demo)
  better   Tavily, if TAVILY_API_KEY (or YAR_SEARCH_API_KEY) is set
...
def _tavily(query: str, key: str, max_results: int) -> list[tuple[str, str, str]]:
    ...
    return [
        (r.get("title", ""), (r.get("content", "") or "")[:400], r.get("url", ""))
...
def make_tool() -> Tool:
    def search_web(query: str, max_results: int = 5) -> str:
        # TAVILY_API_KEY is a documented call-site env (ARCHITECTURE §9), not on Settings.
        ...
        try:
            ...
            if key:
                return (
                    f"Web search failed ({exc}). Answer from what you know, or ask the user."
                )
            results = []
        if not results:
            if not key:
                return (
                    "No results — DuckDuckGo's free endpoint often blocks automated "
