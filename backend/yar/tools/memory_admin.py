"""Tools that let the agent manage its OWN memory.
  ...
  manage_memory  — search / update / delete facts and episodes
  update_soul    — append a durable behaviour rule to SOUL.md
  create_skill   — write a new SKILL.md so the agent builds procedures

Everything writes to the same local files the dashboard shows.
"""
...
SOUL_MAX = 8000
# Skill *names* are intentional English slugs — not a user-text tokenizer.
...
def make_manage_memory_tool(memory) -> Tool:
    ...
    def manage_memory(
        ...
        if action == "search":
            if kind == "episode":
                ...
                if query:
                    ...
                    rows = [
                        r for r in rows if q in normalize(r["summary"]).casefold()
                    ]
                return (
                    "\n".join(
                        f"#{r['id']} ({r['happened_at']}) {r['summary']}"
                        for r in rows[:8]
                    )
                    or "no episodes"
                )
        ...
        if action == "delete":
            if kind == "episode":
                return (
                    f"Deleted episode #{id}."
                    if episodes.delete(int(id))
                    else f"No episode with id {id}."
                )
            return (
                f"Deleted fact #{id}."
                if facts.delete(int(id))
    ...
    return Tool(
        ...
        input_schema={
            ...
            "properties": {
                ...
                "id": {
                    "type": "integer",
                    "description": "row id from a prior search",
                },
...
def make_update_soul_tool(settings) -> Tool:
    from yar.runtime.session import load_soul
    ...
    return Tool(
        ...
        input_schema={
            ...
            "properties": {
                "rule": {
                    "type": "string",
                    "description": "one behaviour rule, imperative",
                }
            },
            "required": ["rule"],
        },
    ...
    return Tool(
        ...
        input_schema={
            ...
            "properties": {
                "name": {
                    "type": "string",
                    "description": "short slug, e.g. weekly-review",
                },
                "description": {
                    "type": "string",
                ...
                "body": {
                    "type": "string",
                    "description": "the step-by-step instructions (markdown)",
                },
            },
