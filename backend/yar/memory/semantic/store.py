...
from yar.text import normalize, tokens


def _fts_query(text: str) -> str:
    """User text isn't a valid FTS5 query (quotes/punctuation break MATCH).
    Reduce it to `word OR word OR ...` via the Unicode tokenizer (§7)."""
    words = tokens(text)
...
class SqliteFactStore:
    ...
    def add(self, subject: str, content: str, source: str = "user") -> None:
        self.conn.execute(
            ...
            (normalize(subject).lower().strip(), normalize(content), source),
        )
        self.conn.commit()
    ...
    def update(self, fact_id: int, content: str, subject: str | None = None) -> bool:
        ...
        if subject is None:
            ...
            cur = self.conn.execute(
                ...
                (content, normalize(subject).lower().strip(), fact_id),
            )
        self.conn.commit()
