"""Skill installer — try anyone's skill in one command.
    ...
    uv run yar skill install https://github.com/<user>/<repo>/blob/main/skills/foo/SKILL.md
    uv run yar skill install https://gist.github.com/<user>/<id>

Downloads the SKILL.md, validates the frontmatter (same check the loader uses),
and drops it in YAR_HOME/skills/<name>/ where the loader picks it up on next
start. Skills are markdown — read what you install.
"""
...
from yar.memory.procedural.loader import _parse
...
def install(url: str) -> None:
    ...
    if skill is None:
        ...
        raise SystemExit(
            ...
            "`description`. See skills/TEMPLATE.md in the repo."
        )
    ...
    print("It loads next time Yar starts. Read it first — skills are instructions.")
