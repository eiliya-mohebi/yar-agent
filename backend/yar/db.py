"""One SQLite file (state.db) holds everything Yar remembers and does.

This mirrors the Hermes approach on the whiteboard: SQLite + FTS5, no server.
Open it yourself anytime:  sqlite3 .yar/state.db '.tables'
"""
...
def _migrate(conn: sqlite3.Connection) -> None:
    ...
    if "source" not in cols:
        # which gateway a message came in through (cli / dashboard)
