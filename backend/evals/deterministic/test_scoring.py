"""DETERMINISTIC EVAL — the ONE Completion scorer (yar.ops.scoring).
...
def test_expect_in_args_substring():
    ...
    assert scoring.check_case(
        case, [{"tool": "create_event", "args": {"title": "Dinner w/ Sam"}}]
    )[0]
    ok, why = scoring.check_case(
        case, [{"tool": "create_event", "args": {"title": "lunch"}}]
    )
...
def test_case_for_message_matches_trimmed_input():
    cases = [
        {"id": "a", "input": "Block three 25-minute focus sessions tomorrow morning"}
    ]
    assert (
        scoring.case_for_message(
            "  Block three 25-minute focus sessions tomorrow morning ", cases
        )["id"]
        == "a"
    )
...
def test_every_flagship_create_event_case_has_fa_counterpart():
    """§7 / issue 12: calendar flagship cases carry a Persian `-fa` twin."""
    cases = {c["id"]: c for c in scoring.load_cases()}
    flagship = [
        cid
        for cid, c in cases.items()
        if c.get("expect_tool") == "create_event" and not cid.endswith("-fa")
    ]
    assert flagship, "expected at least one English create_event flagship case"
    for cid in flagship:
        fa = f"{cid}-fa"
        assert fa in cases, f"missing Persian counterpart {fa}"
        assert cases[fa]["expect_tool"] == "create_event"
